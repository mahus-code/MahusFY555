...
# Add text to animation that displays current time
current2dTime = ax_2d.text(0.75, 0.05, '', transform = ax_2d.transAxes, ha='center')
...
plt.errorbar(time_x[::20], mean_x[::20], yerr=std_x[::20]) # slice : -> from start, :-> to end, 20 -> every 20th point
ax_mean.set_xlabel('Time (s)')
ax_mean.set_ylabel('Mean x coordinate')
ax_mean.set_title('Mean x coordinate versus time (s) of a N-body simulation')
ax_mean.grid(True)
...
def animate_2d(frame):
	...
	current2dTime.set_text(f'Current time: {round(frame*dt, 2)} s') # update the text, formatted string, round to avoid lots of decimals
...
print(f"Time for the quick N-body process: {end_time_quick-start_time_quick:.2f} seconds")	
print(f"Time for the slow N-body process: {end_time_slow-start_time_slow:.2f} seconds")
